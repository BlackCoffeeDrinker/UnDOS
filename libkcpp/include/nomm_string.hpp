...
struct basic_nomm_string {
  ...
  friend constexpr bool operator==(const basic_nomm_string &lhs, T &&rhs) noexcept {
    return static_cast<basic_string_view<CharT, Traits>>(lhs) == static_cast<T &&>(rhs);
  }
  template<class T, class _RT = remove_reference_t<T>, class = enable_if_t<is_pointer_v<_RT>>>
  friend constexpr bool operator==(T &&lhs, const basic_nomm_string &rhs) noexcept {
  ...
  template<size_t M>
  friend constexpr bool operator==(const basic_nomm_string &lhs, const CharT (&rhs)[M]) noexcept {
    return static_cast<basic_string_view<CharT, Traits>>(lhs) == basic_string_view<CharT, Traits>(rhs);
  }
  template<size_t M>
  friend constexpr bool operator==(const CharT (&lhs)[M], const basic_nomm_string &rhs) noexcept {
