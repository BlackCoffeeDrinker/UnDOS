...
inline _KSTD_API _Size __loadword(const void *__p) {
  _Size __r;
  kstd::memcpy(addressof(__r), __p, sizeof(__r));
  return __r;
}
...
struct __murmur2_or_cityhash<_Size, 64> {
  ...
  _KSTD_API static _Size __rotate(_Size __val, int __shift) {
    return __shift == 0 ? __val : ((__val >> __shift) | (__val << (64 - __shift)));
  }

  _KSTD_API static _Size __rotate_by_at_least_1(_Size __val, int __shift) {
    return (__val >> __shift) | (__val << (64 - __shift));
  }

  _KSTD_API static _Size __shift_mix(_Size __val) { return __val ^ (__val >> 47); }

  _KSTD_API static _Size __hash_len_16(_Size __u, _Size __v) {
  ...
  _KSTD_API static _Size
  __hash_len_0_to_16(const char *__s, _Size __len) {
    if (__len > 8) {
      const _Size __a = __loadword<_Size>(__s);
      const _Size __b = __loadword<_Size>(__s + __len - 8);
      return __hash_len_16(__a, __rotate_by_at_least_1(__b + __len, static_cast<int>(__len))) ^ __b;
    }
    if (__len >= 4) {
    ...
    if (__len > 0) {
      ...
      const uint32_t __z = static_cast<uint32_t>(__len) + (static_cast<uint32_t>(__c) << 2);
  ...
  _KSTD_API static pair<_Size, _Size>
...
[[__gnu__::__pure__]] _LIBCPP_EXPORTED_FROM_ABI size_t __hash_memory(_LIBCPP_NOESCAPE const void *, size_t) noexcept;
#else
_KSTD_API inline size_t __hash_memory(const void *__ptr, size_t __size) noexcept {
...
struct __scalar_hash<_Tp, 0> : public __unary_function<_Tp, size_t> {
  _KSTD_API size_t operator()(_Tp __v) const noexcept {
    union {
      _Tp __t;
      size_t __a;
    } __u;
...
struct __scalar_hash<_Tp, 2> : public __unary_function<_Tp, size_t> {
  _KSTD_API size_t operator()(_Tp __v) const noexcept {
    ...
    __u.__t = __v;
    return __hash_memory(addressof(__u), sizeof(__u));
  }
};
...
_KSTD_API inline size_t __hash_combine(size_t __lhs, size_t __rhs) noexcept {
...
template<class _Tp>
struct __hash_impl<_Tp, enable_if_t<is_enum<_Tp>::value && __is_unqualified_v<_Tp>>>
...
struct __hash_impl<
    _Tp,
    enable_if_t<is_integral<_Tp>::value && __is_unqualified_v<_Tp> && (sizeof(_Tp) <= sizeof(size_t))>>
    : __unary_function<_Tp, size_t> {
  _KSTD_API size_t operator()(_Tp __v) const noexcept { return static_cast<size_t>(__v); }
};
...
template<class _Tp>
struct __hash_impl<_Tp, enable_if_t<is_floating_point<_Tp>::value && __is_unqualified_v<_Tp>>>
    : __scalar_hash<_Tp> {
  _KSTD_API size_t operator()(_Tp __v) const noexcept {
...
struct __hash_impl<long double> : __scalar_hash<long double> {
  _KSTD_API size_t operator()(long double __v) const noexcept {
    // -0.0 and 0.0 should return same hash
    if (__v == 0.0L)
      return 0;
...
struct hash<nullptr_t> : public __unary_function<nullptr_t, size_t> {
  [[__nodiscard__]] _KSTD_API size_t operator()(nullptr_t) const noexcept { return 662607004ull; }
};
