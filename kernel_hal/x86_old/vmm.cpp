...
UNDOS_HAL_API bool HAL_VMM_MapPage(uintptr_t virtual_addr, uintptr_t physical_addr, kernel::vmm::ProtectFlags flags) noexcept {
  const uintptr_t pd_index = virtual_addr >> 22;
  ...
  if (auto *directory = hal::x86::get_page_directory();
    ...
    const uintptr_t new_table_phys = HAL_PMM_Allocate_Frames(1);
    if (!new_table_phys) {
      return false;
    }
  ...
  page_table[pt_index].set_frame(physical_addr, hal::x86::translate_flags(flags));

  return true;
}

UNDOS_HAL_API void HAL_VMM_UnmapPage(uintptr_t virtual_addr) noexcept {
  const uintptr_t pd_index = virtual_addr >> 22;
  const uintptr_t pt_index = (virtual_addr >> 12) & 0x3FF;
  ...
  if (auto *directory = hal::x86::get_page_directory();
    ...
    if (table_is_empty) {
      ...
      const uintptr_t page_table_phys = directory[pd_index].get_page_table();
      ...
      HAL_PMM_Free_Frame(page_table_phys);
...
UNDOS_HAL_API void HAL_VMM_Flush(uintptr_t virtual_addr) noexcept {
